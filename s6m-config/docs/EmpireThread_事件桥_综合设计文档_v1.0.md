...
当前 EmpireThread（`empire_thread.py`，523 行，14 种事件类型）已实现事件写入 JSONL（append-only），但缺失**主动 fan-out**——A2A 任务完成、Kanban 操作发生后，事件写入 JSONL 就结束了。Hindsight / Obsidian / MEMORY.md / Session DB 完全不感知，记忆闭环断裂。
...
EmpireThread JSONL（已实现 ✅）
    ...
    ▼
EventBridge（新增）
    ├── Sink: MEMORY.md      ← boot-critical 事实
    ├── Sink: Obsidian        ← 知识库文档（ADR / 事件日志）
    ├── Sink: Hindsight       ← 长期语义记忆
    └── Sink: Session DB      ← 会话历史（改造为事件驱动）
```
...
| # | 加固项 | 不做会怎样 |
|---|--------|-----------|
| **G1** | dispatch 必须异步 out-of-band | Hindsight 抖动拖垮所有工具调用 +200ms |
| **G2** | 事件类型倒排索引 + cursor 增量消费 | 10K 事件起 O(N²) 退化秒级阻塞 |
| **G3** | `_source` 白名单 + 重入计数 | Sink 写回被误识 → 指数级事件膨胀 |
| **G4** | MEMORY.md `flock` + 原子 rename | boot context 被并发写空 |
| **G5** | 队列强制落盘 `pending.jsonl` | 崩溃丢事件，违反 12-Factor VI |
...
**实现顺序**：MEMORY（P0，本地优先）→ Obsidian（P1）→ Hindsight（P2，外部服务）→ Session DB（P3，重构）
...
5. `test_hindsight_5xx_retry_then_dlq` — 3 次重试失败入死信
...
| # | 风险 | 概率 | 影响 | 缓解 |
|---|------|:--:|:--:|------|
| R1 | Sink 自触发死循环 | H | H | `_source` 白名单 + threading.local 重入计数 >1 abort |
| R2 | MEMORY.md 并发写损坏 | H | H | `fcntl.flock` + tmp + `os.replace` 原子写 |
| R3 | Hindsight 网络雪崩 | M | H | 2s 硬超时 + circuit breaker（3 次失败 OPEN 60s） |
| R4 | JSONL 半行写入 | M | M | 推进前校验末字节 `\n`，否则回退 |
| R5 | Sink 慢消费阻塞 | M | M | queue 满即落盘，主路径不阻塞 |

**Hindsight 四层降级**：L0 实时(2s) → L1 重试队列(1/4/16s) → L2 DLQ → L3 熔断跳过(60s)
...
pre_tool_call hook → EmpireThread.append → JSONL fsync（hook 结束）
                              ...
                              event_bridge_daemon（launchd KeepAlive=true）
                                   ├─ 读 cursor，增量消费
                                   └─ 每 Sink：enqueue pending.jsonl
```

**文件**：`~/.hermes/bin/event_bridge_daemon.py`，`~/Library/LaunchAgents/com.hermes.eventbridge.plist`
